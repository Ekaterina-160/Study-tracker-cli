...
| Установка | Скачать [ollama.com](https://ollama.com), `ollama pull llama3.2:1b` | `pip install openai`, получить API-ключ |
...
Ollama рассчитан на запуск **без видеокарты** — модель считается на обычном CPU. Ноутбука с i5 достаточно, GPU не нужен. Главное — оперативная память.

| Модель | Скачать | ОЗУ при работе | Ощущения на i5 без GPU |
|---|---|---|---|
| `llama3.2:1b` | `ollama pull llama3.2:1b` | ~2 ГБ | Быстро, ответ почти сразу — **рекомендуется для курса** |
| `llama3.2` (3B) | `ollama pull llama3.2` | ~4 ГБ | Работает, но ответ «набегает» несколько секунд |

- **ОЗУ важнее процессора.** Минимум 8 ГБ, комфортно — 16 ГБ. Если памяти мало, бери версию `1b`.
- Поколение i5 почти не влияет — подойдёт любой i5 последних лет.
- Первый ответ всегда медленнее (модель грузится в память), дальше быстрее.

Для нашей задачи (короткий совет по готовым метрикам) качества `llama3.2:1b` хватает с запасом, а ждать не придётся. Если ответ всё же приходит долго или модель не отвечает — сработает fallback по локальным правилам (см. Час 2).
...
2. (Вариант A) Установи Ollama, скачай лёгкую модель и убедись, что сервер отвечает:
   ...
   ollama run llama3.2:1b "Привет! Ответь одним словом: работаешь?"
   ```
